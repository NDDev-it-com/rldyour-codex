...
- `scripts/install_system_codex.sh --apply` installs global Codex instructions, managed `~/.codex/agents/*.toml`, managed `~/.codex/rules/*.rules`, config sections, the official Codex config schema hint, `[features].hooks = true`, `[features].multi_agent = true`, removed/deprecated hook alias cleanup, marketplace-derived rldyour plugin enablement, `.mcp.json`-derived MCP materialization into native `[mcp_servers.*]` TOML, YOLO/model defaults, approved MCP tool overrides, marketplace registration, versioned plugin cache under `~/.codex/plugins/cache/rldyour-codex/<plugin>/<version>`, and trusted hashes for installed rldyour plugin hooks through `hooks/list`. It must not write `[features].plugin_hooks`. Add `--strict-runtime` when enabled MCP launchers must be present.
- `config/rldyour-contract.json` and `docs/contract-matrix.md` define the Codex adapter surface: 9 plugins, 39 skills, 0 slash commands by design, 8 managed subagents, command-only plugin hook lifecycle mappings, 13 MCP servers, versioned plugin cache, and owner-local-only YOLO boundaries. Validate with `python3 scripts/validate_contract.py`.
- `scripts/doctor_system_codex.sh` verifies installed state, including marketplace-derived rldyour plugin enablement, `.mcp.json`-derived MCP materialization, the config schema hint, active `hooks` and `multi_agent` features, absence of removed/deprecated hook aliases including `plugin_hooks`, managed subagent config parity, managed execpolicy rule parity, managed subagent `gpt-5.5`/`medium` settings, managed subagent temporary MCP isolation with complete disabled transport metadata, and installed rldyour plugin hook trust/enabled state from `hooks/list`. Use `--quick --strict-runtime` for bounded strict runtime validation.
