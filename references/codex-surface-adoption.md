...
- Official changelog and config docs: `https://github.com/openai/codex/releases/tag/rust-v0.144.1` and `https://developers.openai.com/codex/changelog`

## Decisions

| Surface | Introduced | Decision | Implementation | Validator |
| --- | --- | --- | --- | --- |
| Codex CLI runtime baseline | 0.144.1 | Adopted | Root contract and adapter runtime pins require `@openai/codex` / `codex-cli` `0.144.1`; local installed runtime must report `codex-cli 0.144.1`. The stable patch fixes standalone release-metadata parsing, exposes the code-mode host in macOS package installs, and falls back to the embedded runtime when the companion host is unavailable; it requires no adapter schema migration. | `python3 scripts/check_mcp_runtime_versions.py --fail-on-outdated` |
| Centrally managed startup updates | current docs | Adopted | The installer writes `check_for_update_on_startup = false` to the base, `rldyour-yolo`, and `rldyour-safe` configs. The bootstrap owns the exact Codex runtime pin and upgrade transaction, so Codex must not target an unrelated global npm prefix during startup. Existing user values are migrated idempotently. | `tests/unit/test_install_system_codex_update_policy.py`; `scripts/doctor_system_codex.sh --quick --strict-runtime` |
...
| Stable release boundary vs. prereleases | 0.144.1 | Adopted | Stable `0.144.1` is the release-grade baseline. Prerelease tags remain excluded unless the owner explicitly enables prerelease runtime policy. | `scripts/check_mcp_runtime_versions.py --fail-on-outdated` |
