...
- Official changelog and config docs: `https://github.com/openai/codex/releases/tag/rust-v0.144.0` and `https://developers.openai.com/codex/changelog`

## Decisions

| Surface | Introduced | Decision | Implementation | Validator |
| --- | --- | --- | --- | --- |
| Codex CLI runtime baseline | 0.144.0 | Adopted | Root contract and adapter runtime pins require `@openai/codex` / `codex-cli` `0.144.0`; local installed runtime must report `codex-cli 0.144.0`. The stable release adds interactive MCP authentication, writes-only app approval, runtime-provided app-server authentication, and reliability fixes without requiring a repository schema migration. | `python3 scripts/check_mcp_runtime_versions.py --fail-on-outdated` |
...
| Stable release boundary vs. prereleases | 0.144.0 | Adopted | Stable `0.144.0` is the release-grade baseline. Prerelease tags remain excluded unless the owner explicitly enables prerelease runtime policy. | `scripts/check_mcp_runtime_versions.py --fail-on-outdated` |
