...
- Official changelog and config docs: `https://github.com/openai/codex/releases/tag/rust-v0.142.5` and `https://developers.openai.com/codex/changelog`

## Decisions

| Surface | Introduced | Decision | Implementation | Validator |
| --- | --- | --- | --- | --- |
| Codex CLI runtime baseline | 0.142.5 | Adopted | Root contract and adapter runtime pins require `@openai/codex` / `codex-cli` `0.142.5`; local installed runtime must report `codex-cli 0.142.5`. | `python3 scripts/check_mcp_runtime_versions.py --fail-on-outdated` |
...
| Stable release boundary vs. prereleases | 0.142.5 | Adopted | Stable `0.142.5` is the release-grade baseline. `0.143.0-alpha.*` and later alpha tags remain excluded unless the owner explicitly enables prerelease runtime policy. | `scripts/check_mcp_runtime_versions.py --fail-on-outdated` |
