...
5. Run `instruction-docs-sync` when durable project instructions may have changed and `project_flow_policy.py` reports `instruction_docs.mode` is not `disabled`. Keep `AGENTS.md` optimized for Codex and `.claude/CLAUDE.md` optimized for Claude Code in fullrepo-managed or tracked-normal-branch projects according to policy.
...
10. Publish `fullrepo` only when policy requires/allows it, using the resolved `fullrepo_sync.py --publish` or repo-local helper. Missing fullrepo creation requires explicit policy (`create_if_missing=true`) or explicit current user instruction.
