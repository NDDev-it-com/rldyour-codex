---
name: cmux-worker
description: "Воркер cmux v3 для Codex: cmux worker role, typed task envelope, heartbeat, scoped JSON report, no push. Используй для: cmux worker, worker report. EN triggers: cmux v3 worker. EN: cmux v3 worker."
---
...
- Do not push, force-push, tag, publish, delete branches, mutate project policy, run system install, or run final sync.
