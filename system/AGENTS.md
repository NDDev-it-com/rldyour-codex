...
System Codex installs the safe public profile by default with the official Codex config schema hint, `approval_policy = "on-request"`, `sandbox_mode = "workspace-write"`, `profile = "rldyour-safe"`, `model = "gpt-5.5"`, `model_reasoning_effort = "xhigh"`, `suppress_unstable_features_warning = true`, managed subagents on `gpt-5.5`/`medium` with temporary specialist-MCP isolation, and `[features].hooks = true`, `[features].plugin_hooks = true`, plus `[features].multi_agent = true`. Owner-controlled YOLO execution is explicit: `scripts/install_system_codex.sh --apply --owner-mode` selects `approval_policy = "never"`, `sandbox_mode = "danger-full-access"`, `default_permissions = ":danger-no-sandbox"`, and `profile = "rldyour-yolo"` for the maintainer's trusted local machine only. Current Codex documentation treats `sandbox_mode` as the active older sandbox model when present, so do not migrate this owner profile to beta permission profiles without an explicit policy decision. Current Codex CLI keeps plugin-bundled hooks behind `plugin_hooks`, so the flag is intentionally explicit. Deprecated config aliases such as `codex_hooks`, legacy `features.web_search*`, `experimental_instructions_file`, `background_terminal_timeout`, `experimental_use_unified_exec_tool`, `memories.no_memories_if_mcp_or_web_search`, and `use_legacy_landlock` must not be present. Continue to avoid destructive actions unless the owner explicitly requests them.
