...
- `scripts/smoke_clean_bootstrap.sh`: clones the current repository, runs `scripts/sync_fullrepo_branch.sh --bootstrap-init` to restore agent-only context, installs into a temporary `CODEX_HOME`, and then runs strict doctor/validation checks.
