...
- `${CODEX_HOME:-$HOME/.codex}/config.toml`: active local marketplace registration, YOLO permission defaults, enabled plugin list, and MCP runtime config for system Codex.
...
`system/AGENTS.md` exists and is installed into `${CODEX_HOME:-$HOME/.codex}/AGENTS.md` on the current machine. It is a compact global Codex router/policy file, not a full copy of every plugin workflow.
