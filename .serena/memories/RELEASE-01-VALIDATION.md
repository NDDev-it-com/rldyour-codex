...
- Current product/config version is `1.1.2`; `VERSION`, `pyproject.toml`,
  `uv.lock`, plugin manifests, and `CHANGELOG.md` are the source of truth for
  the adapter-local SemVer state.
- Release `1.1.2` localizes all Codex skill `agents/openai.yaml` UI metadata
  to Russian-first wording, keeps the Codex CLI runtime baseline at `0.135.0`,
...
## Historical evidence
- Release `1.1.0` adopted Codex CLI `0.135.0`, refreshed
  `GITHUB_MCP_SERVER_VERSION=1.1.0`, kept the legacy sandbox permission
  dialect unchanged, and published GitHub Release `1.1.0`.
- Release `1.1.1` completed the Codex `0.135.0` surface-adoption notes for
...
- Commit `818d3c19388978564b29724488678cd803b99867` bumped the product/config
  ...
  tags during manual release runs, and published GitHub Release `1.0.3`.
- Commit `a13c0a18275af27a0148b9ccf01a893d77344503` bumped the product/config
  ...
  published GitHub Release `1.1.0`.
