...
- Release `0.3.1` is published from commit `a9a66a2` (`VERSION=0.3.1`) through manual release run `25996507160`. The GitHub Release tag is `0.3.1`, published at `2026-05-17T16:34:55Z`, with `release-manifest.json`, `release-notes.md`, `rldyour-codex-0.3.1.tar.gz`, `sbom.spdx.json`, and `SHA256SUMS` assets. The release keeps changelog coverage in `CHANGELOG.md`, deterministic release manifest/SBOM validation, manual-only CI workflows, strict runtime prerequisite gates, ordered lifecycle hook behavior, managed execpolicy rules, bounded hook smoke, CODEX_HOME-aware flow state, and clean-runner MCP safe-call stderr classification.
