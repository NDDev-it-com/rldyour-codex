...
- Release `0.3.0` is prepared at commit `2ee72cf` (`VERSION=0.3.0`) with changelog coverage in `CHANGELOG.md`, deterministic release manifest/SBOM validation, manual-only CI workflows, strict runtime prerequisite gates, and ordered lifecycle hook behavior.
