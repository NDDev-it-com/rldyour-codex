...
- checked by: Codex structural-loop repair

## Facts
- Serena memories record memory format, evidence, freshness, fullrepo, and runtime marker policy.
- Fullrepo publish freshness is represented by a tracked overlay state manifest `.rldyour/fullrepo-state.json` generated by `plugins/rldyour-flow/scripts/fullrepo_sync.py`; no-op publish decisions must compare both sparse agent content and the bound base HEAD.
