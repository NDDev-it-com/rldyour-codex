...
- GitHub CI has split unit-test reports, marketplace/system smoke, dependency pin checks, no-paid static security, and manual release workflows with deterministic bundle/SBOM/attestation output; release validation bootstraps `fullrepo` before requiring agent instruction docs and runs PyYAML-dependent agent-surface validation through `uv`.
